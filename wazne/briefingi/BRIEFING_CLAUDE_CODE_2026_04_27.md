...
## KONTEKST — co robiłem dzisiaj lokalnie

Poprzednia sesja (Ty + Claude Code CLI) zidentyfikowała 6 bugów RAG. Ty zrobiłeś część na VPS (masz 4 commity których nie ma na GitHubie — sprawdź `git log --oneline` vs `git log origin/main`). Ja dzisiaj zrobiłem resztę lokalnie na Windowsie — ale moje zmiany nie są na VPS.

Poniżej dokładnie **co brakuje na VPS** (czego Ty jeszcze nie robiłeś). Porównałem Twoje commity z tym co widzę lokalnie — te 4 zmiany są wyłącznie moje.

Testy: `test_astra_behaviors.py` (17 testów) — lokalnie 17/17 PASS. Po naniesieniu zmian uruchom je na VPS.
...
## Co dodać na VPS (brakuje tych zmian)

### 1. `backend/vector_store.py` — 3 zmiany

**a) EXCLUDED_SOURCES w Kanale 1 — user_message_raw exclusion**

W metodzie `search_memories()`, Kanał 1 — zamień filtrowanie na:
```python
...
```
Powód: `user_message_raw` wracało w RAG z similarity ~0.965 (ten sam styl pisania usera) i wypychało wartościowe wspomnienia.
...
if is_milestone:
    final_score += 0.5
```
Powód: VPS ma już `half_life=permanent` dla milestony — nie blakną. +1.0 było podwójnym uprzywilejowaniem.
...
### 2. `backend/semantic_extractor.py` — CORRECTION_KEYWORDS

VPS ma już `FACT:correction` subtype w ENTITY_DEFINITIONS. Brakuje twardego blokowania MILESTONE gdy użytkownik koryguje błąd AI.

**a) Dodaj stałą globalną (przed `extract_persons()`):**
```python
CORRECTION_KEYWORDS = {
...
**b) Użyj w `_find_best_match()` — na początku pętli:**
```python
...
for entity_type, subtypes in self.category_embeddings.items():
    ...
    for subtype, data in subtypes.items():
        if entity_type == 'MILESTONE' and is_correction:
            continue  # korekty idą jako FACT:correction, nie MILESTONE
        # reszta bez zmian
```
Powód: bez tego "To nieprawda, nie mówiłem żebyś mi robiła Earl Grey" → `MILESTONE:vulnerability` zamiast `FACT:correction`.

---

### 3. `backend/main.py` — timestamp prefix w memory block

W funkcji `build_system_prompt()`, w pętli `for mem in fitted:`, dodaj prefix z wiekiem wspomnienia:
...
Dodaj `{time_prefix}` przed `{mem['text']}` w `memory_lines.append(...)`.
Upewnij się że `from datetime import datetime` jest w globalnych importach modułu.

---

### 4. `backend/memory_enricher.py` — FACT:correction

**a) W `IMPORTANCE_RULES['FACT']`:**
```python
'correction': 8,
```

**b) W `SUPERSEDABLE_TOPICS`:**
```python
'correction': 'topic:fact_correction',
```
...
```
Oczekiwany wynik: **17/17 (100%)**. Nie restartuj serwisu dopóki testy nie przejdą.

Plik `test_astra_behaviors.py` jest w lokalnym repo — skopiuj go na VPS lub pobierz po pull z GitHub.
