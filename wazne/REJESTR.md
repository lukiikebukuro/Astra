...
- [x] **G-1** ✅ **ASTRA GOŚĆ — WDROŻONE 24.09 21:18** (`735063c`, deploy potwierdzony: health OK, `/api/siostry/gosc` → `wlaczone:true`, 0 błędów w journalu; flaga `ASTRA_GOSC=on`, rollback = `off` + restart) — przycisk w pokoju sprowadza Astrę na 6 tur z **pełną własną pamięcią** (wektory, `fact_store`, stan, okno RAW 48 h z rozmów solo). **READ-ONLY:** ekstrakcja wyłączona dla całej tury (analog `Fix B7`), zero `add_memory`. Router nietknięty — gość wchodzi obok `routing`, `router_golden` 24/24 bit w bit. Pamięć wizyty powstaje ręcznie: `POST /api/siostry/pamiatka` (endpoint, NIE skrypt — zasada z 25.07). **To wizyta, nie crossover** — „obie pamiętają" przez ekstrakcję pozostaje osobną trudną fazą (recenzja 17.07 pkt 16). Work-order z mapą klasy problemu: `polecenia/work-order_astra_gosc_2026-09-24.md`
...
- [x] **S-9** ✅ **ZROBIONE 24.09** — `parse_gemini_response` dostał parametr `kto` (`main.py:1240`), wszystkich 7 wołających podaje swoją personę: `astra` · `amelia` · `wspolny:<persona>` · `siostry:<imie>` · `siostry:gosc-astra` · `amnezja:<persona>` (×2). Tag Astry zostaje **dosłownie `[ASTRA RAW]`**, więc stare grepy dalej działają — ale przestają łapać 61% cudzych linii. Zrobione przy okazji gościa, bo bez tego eksperymentu nie dałoby się odczytać z logów. *Pierwotny opis:* `[ASTRA RAW]` oznaczał tury wszystkich person — 61% linii (148 z 243) to nie Astra.
