...
- [x] **G-1** ✅ **ASTRA GOŚĆ — zbudowane 24.09** (lokalnie, **niezdeployowane**, flaga `ASTRA_GOSC=off`) — przycisk w pokoju sprowadza Astrę na 6 tur z **pełną własną pamięcią** (wektory, `fact_store`, stan, okno RAW 48 h z rozmów solo). **READ-ONLY:** ekstrakcja wyłączona dla całej tury (analog `Fix B7`), zero `add_memory`. Router nietknięty — gość wchodzi obok `routing`, `router_golden` 24/24 bit w bit. Pamięć wizyty powstaje ręcznie: `POST /api/siostry/pamiatka` (endpoint, NIE skrypt — zasada z 25.07). **To wizyta, nie crossover** — „obie pamiętają" przez ekstrakcję pozostaje osobną trudną fazą (recenzja 17.07 pkt 16). Work-order z mapą klasy problemu: `polecenia/work-order_astra_gosc_2026-09-24.md`
